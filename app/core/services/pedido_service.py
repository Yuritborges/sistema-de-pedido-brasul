...
Orquestra a geração do pedido: valida DTO, gera PDF e registra no banco.
"""
...
class PedidoService:
    ...
    def gerar_pdf(self, dto: PedidoDTO) -> str:
        """Valida, gera o PDF e salva o pedido no banco. Retorna o caminho do arquivo."""
        self._validar(dto)
        caminho = self._generator.gerar(dto)
        self._registrar_historico(dto, caminho)
        return caminho

    def _validar(self, dto: PedidoDTO):
        erros = []
    ...
    def _registrar_historico(self, dto: PedidoDTO, caminho_pdf: str):
        """
        Salva o pedido e seus itens no banco de dados SQLite.
        Se o número já existir (pedido reimpresso), atualiza em vez de inserir.
        """
        try:
            ...
            with get_connection() as conn:
                ...
                total_pedido = getattr(dto, "total_liquido", getattr(dto, "total", 0.0))
                ...
                if existente:
                    ...
                    # Remove os itens antigos e reinsere os atuais
                    conn.execute(
            ...
            # Não derruba a geração do PDF por erro de banco
            print(f"[DB] Aviso ao registrar pedido: {e}")